...
def set_loader(opt):
    ...
    if opt.local_modality == 'acc':
        train_dataset = data.Singlemodal_dataset(x_train_1, y_train)
        test_dataset = data.Singlemodal_dataset(x_test_1, y_test)
    elif opt.local_modality == 'gyr':
        train_dataset = data.Singlemodal_dataset(x_train_2, y_train)
        test_dataset = data.Singlemodal_dataset(x_test_2, y_test)
...
def set_model(opt):
    ...
    if opt.local_modality == 'both':
        ...
        model = MySingleModel(input_size=1, num_classes=opt.num_class, modality=opt.local_modality)
