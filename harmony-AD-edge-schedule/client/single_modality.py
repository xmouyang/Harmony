...
def parse_option(selected_modality, selected_usr_id):
    ...
    parser.add_argument('--num_of_train', type=int, default=100,
                        help='num_of_train')
    parser.add_argument('--num_of_test', type=int, default=100,
                        help='num_of_test')
    ...
    parser.add_argument('--time_slice', type=int, default = 100, 
        help='set the total time slice')

    opt = parser.parse_args()

    opt.usr_id = selected_usr_id
    ...
    opt.result_path = './save_unifl_results/node_{}/{}_results/'.format(opt.usr_id, selected_modality)
...
def set_loader(opt, selected_modality):
    ...
    if selected_modality == "all":
        ...
        total_dataset = data.Unimodal_dataset(opt.usr_id, selected_modality)
...
def set_model(opt, selected_modality):

    if selected_modality == "all":
        model = My3Model(num_classes = opt.num_class)
    else:
        model = MySingleModel(num_classes = opt.num_class, modality = selected_modality)
...
def set_commu(opt, selected_modality):
    ...
    elif selected_modality == "depth":
        server_port = 9998#30101#
    elif selected_modality == "radar":
    ...
    comm.send2server(selected_modality ,1)

    return comm


def exec_unimodal_training(selected_modality, connWrite, event, selected_usr_id):
    best_acc = 0
    opt = parse_option(selected_modality, selected_usr_id)

    print("!!!!!!!!!!!!Successfully set the user id as : ", opt.usr_id)

    # set up communication with sevrer
    comm = set_commu(opt, selected_modality)
    
    # build data loader
    train_loader = set_loader(opt, selected_modality)

    # build model and criterion
    model, criterion = set_model(opt, selected_modality)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if (epoch % opt.fl_epoch) == 0:
            ...
            # notify main process this round has been done
            connWrite.send("True")
            ...
            # create a new process to conduct transmission task and get back to main process
            model_update = mp.Process(target = comm.send2server, args=(w_update,0))
            model_update.start()
            event.set()
            commu_epoch = int(epoch/opt.fl_epoch - 1)
            # when using a new process to send wait, measuring communication time is nonsense
